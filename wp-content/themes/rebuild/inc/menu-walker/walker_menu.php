...
$GLOBALS['rebuild_menu_icon'] = new rebuild_menu_icon();
if ( ! function_exists( 'rebuild_fontawesome_icons' ) ) {
	function rebuild_fontawesome_icons() {
 ...
 */
 function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 )
 {
    ...
    if ( ! empty( $item->_invalid ) ) {
        ...
        $title = sprintf( esc_html__( '%s (Invalid)','rebuild' ), $item->title );
    } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
        $classes[] = 'pending';
        /* translators: %s: title of menu item in draft status */
        $title = sprintf( esc_html__('%s (Pending)','rebuild'), $item->title );
    }
            ...
            <dt class="menu-item-handle">
                ...
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','rebuild'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                            echo wp_nonce_url(
                                add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','rebuild'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo esc_attr( $item_id ); ?>" title="<?php esc_attr_e('Edit Menu Item','rebuild'); ?>" href="<?php
                        echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
                    ?>"><?php esc_attr_e( 'Edit Menu Item','rebuild' ); ?></a>
                </span>
            </dt>
        </dl>
        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr( $item_id ); ?>">
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'Navigation Label','rebuild' ); ?><br />
                    <input type="text" id="edit-menu-item-title-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                </label>
            </p>
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'Title Attribute','rebuild' ); ?><br />
            ...
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo esc_attr( $item_id ); ?>">
                    ...
                    <?php esc_html_e( 'Open link in a new window/tab','rebuild' ); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'CSS Classes (optional)','rebuild' ); ?><br />
                    <input type="text" id="edit-menu-item-classes-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
                </label>
            </p>
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'Link Relationship (XFN)','rebuild' ); ?><br />
                    <input type="text" id="edit-menu-item-xfn-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
                </label>
            </p>
            <p class="field-description description description-wide">
                <label for="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'Description','rebuild' ); ?><br />
                    <textarea id="edit-menu-item-description-<?php echo esc_attr( $item_id ); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr( $item_id ); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
                    <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.','rebuild'); ?></span>
                </label>
            </p>        
            <?php
            /*
            ...
            <p class="field-fontawesome description description-thin">
                <label for="edit-menu-item-fontawesome-<?php echo esc_attr( $item_id ); ?>">
                    <?php esc_html_e( 'Select Menu Icon','rebuild' ); ?><br />
                    <input type="text" id="edit-menu-item-fontawesome-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-fontawesome" name="menu-item-fontawesome[<?php echo esc_attr( $item_id ); ?>]" value="<?php echo esc_attr( $item->fontawesome ); ?>" />
            ...
            <ul class="fonticon-group">                
				<?php $icons = rebuild_fontawesome_icons(); ?>
                    <li><i class=""></i><span class="icon-name"></span></li>
                <?php foreach ( $icons as $icon ) { ?>
                    <li><i class="fa fa-<?php echo esc_attr( $icon ); ?>"></i><span class="icon-name">fa-<?php echo esc_attr( $icon ); ?></span></li>
                <?php } ?>
            </ul>
            </div>
		    ...
		    <label for="edit-menu-item-highlight-<?php echo esc_attr( $item_id ); ?>">
		        <?php esc_html_e( 'Menu Title BG Color', 'rebuild' ); ?>
		        <select id="edit-menu-item-highlight-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-highlight" name="menu-item-highlight[<?php echo esc_attr( $item_id ); ?>]">					<?php $selected_color = get_post_meta( $item->ID, '_menu-item-highlight', true);?>
		        	<option value="none" <?php if( empty($selected_color) || $selected_color=='none' ) { echo 'selected'; } ?>><?php esc_html_e('Select BG Color', 'rebuild'); ?></option>
		            <option value="jx-rebuild-green" <?php if($selected_color == 'jx-rebuild-green') { echo 'selected'; } ?>><?php esc_html_e('Green','rebuild'); ?></option>
                    <option value="jx-rebuild-red" <?php if($selected_color == 'jx-rebuild-red') { echo 'selected'; } ?>><?php esc_html_e('Red','rebuild'); ?></option>
                    <option value="jx-rebuild-orange" <?php if($selected_color == 'jx-rebuild-orange') { echo 'selected'; } ?>><?php esc_html_e('Orange','rebuild'); ?></option>
                    <option value="jx-rebuild-yellow" <?php if($selected_color == 'jx-rebuild-yellow') { echo 'selected'; } ?>><?php esc_html_e('Yellow','rebuild'); ?></option>
                    <option value="jx-rebuild-blue" <?php if($selected_color == 'jx-rebuild-blue') { echo 'selected'; } ?>><?php esc_html_e('Blue','rebuild'); ?></option>
		        </select>
		    </label>
			</p>
		    ...
		    <label for="edit-menu-item-megamenu-<?php echo esc_attr( $item_id ); ?>">
		        <?php esc_html_e( 'Set Mega Menu', 'rebuild' ); ?>
		        <select id="edit-menu-item-megamenu-<?php echo esc_attr( $item_id ); ?>" class="widefat code edit-menu-item-megamenu" name="menu-item-megamenu[<?php echo esc_attr( $item_id ); ?>]">					<?php $set_megamenu = get_post_meta( $item->ID, '_menu-item-megamenu', true);?>
		        	<option value="no-mega" <?php if( empty($set_megamenu) || $set_megamenu=='no-mega' ) { echo 'selected'; } ?>><?php esc_html_e('No', 'rebuild'); ?></option>
		            <option value="menu-mega" <?php if($set_megamenu == 'menu-mega') { echo 'selected'; } ?>><?php esc_html_e('Yes','rebuild'); ?></option>
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                    <p class="link-to-original">
                        <?php printf( esc_html__('Original: %s','rebuild'), '<a href="' . esc_url( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                    </p>
                <?php endif; ?>
                ...
                ); ?>"><?php esc_html_e('Remove','rebuild'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr( $item_id ); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
                    ?>#menu-item-settings-<?php echo esc_attr( $item_id ); ?>"><?php esc_html_e('Cancel','rebuild'); ?></a>
            </div>
...
/*-----------------Custom Menu -------------------------------*/
	class rebuild_mega_menu extends Walker_Nav_Menu {
	function start_lvl( &$output, $depth = 0, $args = array() ) {
