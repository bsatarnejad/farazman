	...
	class RWMB_Image_Advanced_Field extends RWMB_Image_Field
	{
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_localize_script( 'rwmb-image-advanced', 'rwmbImageAdvanced', array(
				'frameTitle' => esc_html__( 'Select Images', 'rebuild' ),
			) );
		}
		/**
		 * Add actions
		...
		{
			$i18n_title = apply_filters( 'rwmb_image_advanced_select_string', _x( 'Select or Upload Images', 'image upload', 'rebuild' ), $field );
		...
		{
			$i18n_delete = apply_filters( 'rwmb_image_delete_string', _x( 'Delete', 'image upload', 'rebuild' ) );
			$i18n_edit   = apply_filters( 'rwmb_image_edit_string', _x( 'Edit', 'image upload', 'rebuild' ) );
			?>
            <script id="tmpl-rwmb-image-advanced" type="text/html">
				...
				<li id="item_{{{ attachment.id }}}">
					...
					<div class="rwmb-image-bar">
						<a title="<?php echo esc_attr($i18n_edit); ?>" class="rwmb-edit-file" href="{{{ attachment.editLink }}}" target="_blank"><?php echo esc_attr($i18n_edit); ?></a> |
