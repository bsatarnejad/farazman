	...
	class RWMB_File_Advanced_Field extends RWMB_File_Field
	{
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_localize_script( 'rwmb-file-advanced', 'rwmbFileAdvanced', array(
				'frameTitle' => esc_html__( 'Select Files', 'rebuild' ),
			) );
		}
		/**
		 * Add actions
		...
		{
			$i18n_title  = apply_filters( 'rwmb_file_advanced_select_string', _x( 'Select or Upload Files', 'file upload', 'rebuild' ), $field );
		...
		{
			$i18n_delete = apply_filters( 'rwmb_file_delete_string', _x( 'Delete', 'file upload', 'rebuild' ) );
			$i18n_edit   = apply_filters( 'rwmb_file_edit_string', _x( 'Edit', 'file upload', 'rebuild' ) );
			?>
            <script type="text/html" id="tmpl-rwmb-file-advanced">
				...
				<li>
					...
					<div class="rwmb-info">
						...
						<p>{{{ attachment.mime }}}</p>
						<a title="<?php echo esc_attr($i18n_edit); ?>" href="{{{ attachment.editLink }}}" target="_blank"><?php echo esc_attr($i18n_edit); ?></a> |
						<a title="<?php echo esc_attr($i18n_delete); ?>" class="rwmb-delete-file" href="#" data-attachment_id="{{{ attachment.id }}}"><?php echo esc_attr($i18n_delete); ?></a>
					</div>
				</li>
				<# } ); #>
			</script>
