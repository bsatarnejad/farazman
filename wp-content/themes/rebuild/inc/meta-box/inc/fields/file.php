	...
	class RWMB_File_Field extends RWMB_Field
	{
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_localize_script( 'rwmb-file', 'rwmbFile', array(
				'maxFileUploadsSingle' => esc_html__( 'You may only upload maximum %d file', 'rebuild' ),
				'maxFileUploadsPlural' => esc_html__( 'You may only upload maximum %d files', 'rebuild' ),
			) );
		}
		/**
		 * Add actions
		...
		static function wp_ajax_delete_file()
		{
			...
			if ( $ok )
				...
				wp_send_json_error( esc_html__( 'Error: Cannot delete file', 'rebuild' ) );
		}
		/**
		 * Get field HTML
		 *
		...
		{
			$i18n_title = apply_filters( 'rwmb_file_upload_string', _x( 'Upload Files', 'file upload', 'rebuild' ), $field );
			$i18n_more  = apply_filters( 'rwmb_file_add_string', _x( '+ Add new file', 'file upload', 'rebuild' ), $field );
			// Uploaded files
