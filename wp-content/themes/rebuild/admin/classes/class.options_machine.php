	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				//One-Click Demo Importer options data
				//display a button with url
				case "button":
					
					if ( is_plugin_active( 'rebuild-framework/install.php' ) ) {
   				 		$src = $value['std'];
						$output .= '<a href="' . $src . '" class="button-primary import-demo">Import Demo</a>';
					}else{
					
						$output .="You need to activate Rebuild Framework Plugin";	
						}
				...
				case 'backup':
					...
					$output .= '<p><strong>'. esc_html__('Last Backup : ','rebuild').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
