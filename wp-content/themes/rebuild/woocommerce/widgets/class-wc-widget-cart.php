...
class Custom_WC_Widget_Cart extends WC_Widget_Cart {
	...
	function widget( $args, $instance ) {
		...
		global $woocommerce;
		$title         = apply_filters( 'widget_title', empty( $instance['title'] ) ? esc_html__( 'Cart', 'rebuild' ) : $instance['title'], $instance, $this->id_base );
	...
	function form( $instance ) {
		$hide_if_empty = empty( $instance['hide_if_empty'] ) ? 0 : 1;
		?>
		<p><label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_html_e( 'Title:', 'rebuild' ) ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id( 'hide_if_empty' )); ?>"><?php esc_html_e( 'Hide if cart is empty', 'rebuild' ); ?></label>
		</p>
	<?php
	}
}