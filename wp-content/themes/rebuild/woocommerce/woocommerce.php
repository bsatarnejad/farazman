...
if ( ! function_exists( 'woocommerce_template_loop_product_thumbnail' ) ) {
	function woocommerce_template_loop_product_thumbnail() {
		...
		switch ($rebuild_data['woo_image_hover']){
...
function rebuild_get_current_cart_info() {
	global $woocommerce;
...
function rebuild_add_to_cart_success_ajax( $count_cat_product ) {
	global $woocommerce;
	list( $cart_items ) = rebuild_get_current_cart_info();
	if ( $cart_items > 0 ) {
...
add_filter( 'add_to_cart_fragments', 'rebuild_add_to_cart_success_ajax' );
...
add_filter( 'loop_shop_per_page', 'rebuild_loop_shop_per_page' );
function rebuild_loop_shop_per_page() {
	$rebuild_data =  rebuild_globals();
 	parse_str( $_SERVER['QUERY_STRING'], $params );
	if ( isset($rebuild_data['woo_number_per_page']) && $rebuild_data['woo_number_per_page'] ) {
		$per_page = $rebuild_data['woo_number_per_page'];
	} else {
		$per_page = 12;
	}
...
if ( ! function_exists( 'woocommerce_breadcrumb' ) ) {
	...
	function woocommerce_breadcrumb( $args = array() ) {
		$defaults = apply_filters( 'woocommerce_breadcrumb_defaults', array(
			...
			'before'      => '',
			'after'       => '',
			'home'        => _x( 'Home', 'breadcrumb', 'rebuild' ),
		) );
