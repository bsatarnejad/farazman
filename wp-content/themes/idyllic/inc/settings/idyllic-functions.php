		...
		while ($query->have_posts()):$query->the_post();
			...
			$excerpt = get_the_excerpt();
				...
				if($idyllic_settings['idyllic_slider_button'] == 0){
					...
					if($excerpt_text == '' || $excerpt_text == 'ادامه مطلب') :
						$idyllic_category_sliders_display 	.= '<a title='.'"'.the_title_attribute('echo=0'). '"'. ' '.'href="'.esc_url(get_permalink()).'"'.' class="btn-default vivid-red">'.esc_html__('ادامه مطلب', 'idyllic').'</a>';
					else:
