...
function janxcode_importer() 
{
	...
	if ( current_user_can( 'manage_options' ) && isset( $_GET['import_data_content'] ) ) { 
			...
			$theme_xml = plugin_dir_path( __FILE__ ).'/demo/rebuild.xml.gz' ; // Get the xml file from directory 
			...
			if($registered_menus):			
				foreach($registered_menus as $menu) { 
						...
						elseif( $menu->name == 'One-Page Menu' ) :
							$menu_location['onepage_navigation'] = $menu->term_id;							
						endif;
					}	
			endif;
			...
			$data = unserialize($get_theme_options['body']);
            ...
            foreach( $sidebars as $sidebar ) {
                ...
                register_sidebar(array(
                    'name'=>$sidebar,
                    'id' => 'rebuild-custom-sidebar-' . strtolower( $sidebar ),
                    'before_widget' => '<div class="jx-rebuild-sidebar-widget"><div id="%1$s" class="widget %2$s">',
					'after_widget'  => '</div></div>',
					'before_title'  => '<div class="sub"><header><h4>',
					'after_title'   => '</h4><div class="jx-rebuild-right-pattern"></div></header></div>',
                ));
            }
