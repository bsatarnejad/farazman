<?php 
	...
	//=========================================================================//
		...
		function submit_form_callback(){
			...
			$site_owners_email = $rebuild_data['text_email']; // Replace this with your own email address
		
			$body    = esc_html__('Name:', 'rebuild')." $name \n\n"; 
			$body   .= esc_html__('Email:', 'rebuild')." $email \n\n"; 
			$body   .= esc_html__('Message:', 'rebuild')." $message \n\n"; 
		...
		function request_form_callback(){
			...
			$name = trim($params['jx-rebuild-service-type']);
			$email = $params['contact_email'];
			$subject = $params['contact_subject'];
			$message = $params['contact_message'];
